...
#' 
#' @param meta The R object holding Metapop metadata returned by \link{mpmeta}.
...
#' @export
mpkch <- function (meta, path) {
  f <- list.files(path, full.names=TRUE, patt='\\.kch$', ignore.case=TRUE)
  kchs <- path.expand(file.path(path, meta$kch))
  if (any(!file.exists(kchs))) {
    kchs <- f[match(tolower(kchs), tolower(f))]
  } 
  if (any(!file.exists(kchs[!is.na(kchs)]))) stop('Some file paths do not exist - ask John')
