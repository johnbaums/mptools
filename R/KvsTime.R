...
#' @param plotN (logical) If \code{TRUE}, mean population abundance will be plotted (solid lines) in addition to carrying capacity (dashed lines).
#' @param results (required only if \code{plotN} is \code{TRUE}) The R object holding simulation results returned by \link{mpresults}. 
...
#' @export
KvsTime <- function(meta, kch, filename, samelims=FALSE, plotN=FALSE, results, layout=c(5, 10), ...) {
  require(zoo)
  require(lattice)
  ...
  if (isTRUE(plotN)) {
    ...
    print(xyplot(zoo(kchn), screens=rep(colnames(kch), 2),
