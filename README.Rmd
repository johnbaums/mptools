...
evaluating the impacts of habitat change
[@Cadenhead2015;@Fordham2012;@Keith2008;@Keith2014;@Southwell2008]. Frequently,
these coupled models used the Metapop module of RAMAS GIS [@Akcakaya2005] to
relate spatially-dynamic habitat suitability to carrying capacity, and then
...
calculates the expected minimum abundance [EMA; @McCarthy2001b] and the standard
deviation of minimum abundance.
...
`mp2xy`. This requires one of the original grids used by Spatial Data, and
knowledge of the cell length setting passed to that module. By default, `mp2xy`
creates a plot of the points, overlaid upon the provided raster data.

The raster grids that were originally used to define the patch structure are
included with `mptools`. We refer to one these below.

```{r xy, echo=FALSE}
r <- system.file('example_001.asc', package='mptools')
xy <- mp2xy(mp=mp, r=r, cell.length=9.975, plot=FALSE)
head(xy)
```

```{r xy_echo, eval=FALSE}
r <- system.file('example_001.asc', package='mptools')
...
or with `sp::spplot`, which can colour points according to the value of an
attribute:

```{r plot_spplot, echo=c(1, 2, 4), results='hide'}
library(sp)
library(viridis)
svg('README_files/plot_spplot-1.svg')
...
The function requires either: (1) a `RasterStack`, with layers in temporal
order, or (2) a file path to a directory containing raster files that describe
habitat change. In the latter case, filenames must end with an underscore,
followed by a number that indicates the temporal position of the raster in the
time series of habitat dynamics. The interval between successive rasters (i.e.,
the time step) is assumed to be constant. Below, we pass a `RasterStack`,
comprising rasters that are included with `mptools`.
