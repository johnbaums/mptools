...
included with `mptools` as a `RasterStack` object, `habitat`. We refer to this 
below.
...
the interval between time steps) is assumed to be constant. Below, we pass the 
`RasterStack` included with `mptools` (i.e., `habitat`) to `mp_animate`.
